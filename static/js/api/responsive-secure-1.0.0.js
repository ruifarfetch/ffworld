...
if (window.matchMedia && ffbrowser.isIE8 === false) {
    ...
    /// Bigger than fablet - big devices responsive actions
    FFAPI.responsive.goneBigQuerie = window.matchMedia('screen and ' + '(min-width: ' + (FFAPI.responsive.fablet + 1) + 'px)');
    ...
    /// Between mobile and fablet - for the responsive actions
    FFAPI.responsive.goneSmallQuerie = window.matchMedia('screen and ' + '(max-width:' + (FFAPI.responsive.fablet) + 'px)');
    ...
    */
    FFAPI.methods.redirectToSearchResults = function (value, genderID) {
        ...
        if (value) {
            var obj = { searchTerm: encodeURIComponent(value), gender: genderID };
            $.ajax({
                type: "POST",
                ...
                success: function (data) {
                    if (data.hasOwnProperty("d") && data.d != null && data.d != "") {
                        ...
                        value = value.replace(/>/gi, "");
                        var valueHex = FFAPI.methods.stringToHex(value);
                        window.location = homeUrl + "/shopping/" + gender + "/search/schid-" + valueHex + "/items.aspx?q=" + value;
